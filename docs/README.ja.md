...
英語・日本語の導入手順を試す方を3名募集しています。[参加・導入時のフィードバック](https://github.com/coipod/coi/issues/2)。対応するApple Silicon Mac、ソースビルド、ご自身のAI提供元アカウントが必要です。AI利用料が発生する場合があります。

## はじめる

現在はソース配布の開発アルファです。リポジトリは公開されており、組織へのアクセス権は不要です。署名済みインストーラーは配布していません。
